...
from services.face_service import capture_face_from_image
import base64
import numpy as np
...
def verify_face():
    ...
    new_encoding, err = capture_face_from_image(image_bytes)

    if err:
        return jsonify({"error": err}), 400

    known_encoding = np.array(user_data.get("face_embedding"))
